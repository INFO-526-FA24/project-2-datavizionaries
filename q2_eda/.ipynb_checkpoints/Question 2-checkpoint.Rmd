...
ggplot(occupation_distribution, aes(x = occupation, y = count, fill = diagnosed_label)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Yes" = "skyblue", "No" = "lightgray")) +
  theme(
...
auc(roc_curve)
```
# Country over time graph

```{r}
country_over_time <- data_clean %>%
    mutate(record_date = floor_date(timestamp, unit = "month")) %>%
  filter(., diagnosed == 1) %>%
  group_by(record_date, country) %>%
  summarise(count = n(), .groups = "drop")

ggplot(country_over_time, aes(x = record_date, y = count, group = country, color = country)) +
    geom_line() +
    geom_point()
