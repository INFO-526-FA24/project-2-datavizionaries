...
## Question 1: What correlations exist between coping difficulties, social weakness, and work interest in occupation and self-employment status? Does a person's or their family's history of mental health problems link with higher levels of mood swings, coping challenges and social fragility?

### Approach

::: {style="text-align: justify;"}
The initial phase of our analysis focuses on data preprocessing, where we will transform categorical variables including Coping_Struggles, Social_Weakness, Work_Interest, Occupation, and self_employed into factors. A crucial aspect of this phase involves handling missing values through a systematic approach. For features with minimal missing data (less than 5%), we'll apply mean or median imputation. When dealing with moderate levels of missing data (5-30%), we'll implement techniques such as imputation and removing. Features with substantial missing data (exceeding 30%) will be evaluated for their analytical significance and potentially removed if deemed non-essential.

The second phase involves conducting Exploratory Data Analysis (EDA) to uncover patterns and relationships within the data. We will examine how Coping_Struggles, Social_Weakness, and Work_Interest are distributed across different Occupations and employment statuses. Additionally, we'll investigate the relationship between family mental health history and both Coping_Struggles and Social_Weakness to identify potential patterns or correlations.

The final phase focuses on creating informative visualizations to effectively communicate our findings. We'll develop stacked bar plots to illustrate how coping struggles, social weakness, and work interest are distributed across different occupations and employment statuses. A heatmap will be created using dplyr and ggplot2 to display correlations between categorical variables. Lastly, we'll generate bar plots to compare the proportions of coping struggles and social weakness between self-employed and non-self-employed individuals, providing clear visual insights into these relationships.
:::
...
## Question 2: What is the distribution of mental health treatment-seeking across different demographics (e.g., gender, country, occupation)? Also, are there trends seen over time (based on Timestamp), such as increasing stress levels or changes in treatment-seeking behaviors?
